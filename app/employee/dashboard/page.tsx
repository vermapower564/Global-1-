    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16 font-sans">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xs">
        ...
        <div className="flex items-center gap-3">
          <Link href="/employee/tasks" className="bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs px-4 py-3 rounded-2xl shadow-md transition">
            My Task Workboard →
          </Link>
          <Link href="/employee/work" className="bg-gray-100 hover:bg-gray-200 text-black font-extrabold text-xs px-4 py-3 rounded-2xl border border-gray-200 transition">
            + Submit EOD Report
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
        <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-2xs">
          <p className="text-[10px] font-extrabold uppercase text-gray-400">Total Tasks</p>
        ...
        <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-2xs">
          <p className="text-[10px] font-extrabold uppercase text-gray-400">Completion %</p>
      ...
      <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-xs space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-100 pb-3 gap-2">
          <div>
            <h2 className="font-black text-black text-lg tracking-tight">📂 My Assigned Projects & Pending Work</h2>
            <p className="text-xs text-gray-500">Detailed scope, pending work items, consumed hours, and 2-month target completion deadline.</p>
          </div>
        ...
        {loading ? (
          <div className="p-8 text-center text-gray-500 font-bold text-xs">Loading assigned project work...</div>
        ) : todayTasks.length === 0 ? (
          <div className="bg-gray-50 p-8 rounded-3xl border border-gray-200 text-center space-y-2">
            <div className="text-2xl">🎉</div>
            <h3 className="font-extrabold text-black text-sm">No Active Assigned Work</h3>
            <p className="text-xs text-gray-500">You are all caught up for today!</p>
          </div>
        ) : (
          <div className="space-y-4">
            {todayTasks.map((item, idx) => {
              ...
              return (
                <div key={item.id} className="p-5 rounded-3xl border border-gray-200 bg-white space-y-4 shadow-2xs hover:border-blue-300 transition">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase ${
                          ...
                          item.priority === "HIGH" ? "bg-amber-50 text-amber-800 border border-amber-200" :
                  ...
                  <div className="text-xs text-gray-600 space-y-1">
                    <span className="font-extrabold text-black block">Work Details & Scope:</span>
                    <p className="leading-relaxed bg-gray-50/70 p-3.5 rounded-2xl border border-gray-200 font-medium text-gray-700">
